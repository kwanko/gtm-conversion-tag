___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Kwanko Universal Conversion Tag",
  "categories": ["AFFILIATE_MARKETING", "ADVERTISING", "CONVERSIONS"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "Kwanko",
    "thumbnail": "data:image/png;base64,"
  },
  "description": "This tag will give you the opportunity to trigger all kings of conversion supported by Kwanko using our latest tracking features like uniJS and first party.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "trkcode",
    "displayName": "(Mandatory) Your Kwanko conversion identifier (aka the mclic value) provided by Kwanko.",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "eventid",
    "displayName": "(Mandatory) Your unique conversion id.",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ]
  },
  {
    "type": "TEXT",
    "name": "trkdomain",
    "displayName": "(Optionnal) Your Kwanko First Party tracking domain provided by Kwanko.",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "amount",
    "displayName": "(Optionnal) Conversion amount. \nThis value is mandatory for sales.",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "currency",
    "displayName": "(Optionnal) specific currency ISO code associated to the given amount.",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "email",
    "displayName": "[Optionnal) md5 hashed email from user",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "vouchercode",
    "displayName": "(Optionna) Voucher code",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "payname",
    "displayName": "(Optionnal) used payment mode",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "trktype",
    "displayName": "(Optionnal) Tracking type. Possible values are \"conversion\" (by default if not given), \"dclick\" or \"clicktime\".",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "progid",
    "displayName": "(Optionnal) Your Kwanko Program id, only usefull/required for clicktime tracking conversion.",
    "simpleValueType": true
  },
  {
    "type": "TEXT",
    "name": "ptag",
    "displayName": "(Optionnal) Used to fire your Kwanko PartnerTag",
    "simpleValueType": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const getType         = require('getType');
const logToConsole    = require('logToConsole');
const injectScript    = require('injectScript');
const setInWindow     = require('setInWindow');
const callInWindow    = require('callInWindow');
const getCookieValues = require('getCookieValues');

var kwkUniv, kwkDebug, kwkArgs, kwkDoTrk, kwkFirePtag, kwkUniJSConvCall, kwkTrkAction;

kwkUniv  = getCookieValues('kwku').length > 0 ? getCookieValues('kwku').pop() : '';
kwkDebug = getCookieValues('kwkdebug').length > 0;
kwkArgs = {
  'mclic'    : data.trkcode,
  'argmon'   : data.amount,
  'kwkuniv'  : kwkUniv,
  'argann'   : getType(data.eventid) === 'string' ? data.eventid : '',
  'nacur'    : getType(data.currency) === 'string' ? data.currency : '',
  'altid'    : getType(data.email) === 'string' ? data.email : '',
  'argbr'    : getType(data.payname) === 'string' ? data.vouchercode : '',
  'argmodp'  : getType(data.payname) === 'string' ? data.payname : ''
};

const log = kwkDebug ? logToConsole : (() => {});
log('KWKGTM debug - data =', data);
log('KWKGTM debug - args =', kwkArgs);
kwkDoTrk = (getType(data.trkcode) === 'string' && data.trkcode.length > 0) || (getType(data.trktype) === 'string' && data.trktype == 'clicktime');
kwkFirePtag = getType(data.ptag) === 'string' && data.ptag === '1';

if (kwkDoTrk) {
  if (getType(data.trktype) === 'string' && data.trktype == 'dclick') {
    kwkTrkAction = 'KWKUniJS.dclick';
  }
  else if (getType(data.trktype) === 'string' && data.trktype == 'clicktime') {
    kwkTrkAction = 'KWKUniJS.clicktime';
  }
  else {
    kwkTrkAction = 'KWKUniJS.conversion';
  }

  log('KWKGTM debug - kwkTrkAction ='+kwkTrkAction);
  kwkUniJSConvCall = callInWindow(kwkTrkAction, data.trktype === 'clicktime' ? data.progid : kwkArgs);
  log('KWKGTM debug - kwkTrkAction call result='+kwkUniJSConvCall);

  if (getType(kwkUniJSConvCall) !== 'undefined') {
    log('KWKGTM debug - '+kwkTrkAction+' successfully called');
  }
  else {
    kwkArgs.trkdomain = getType(data.trkdomain) === 'string' ? data.trkdomain : '';
    kwkArgs.trktype   = getType(data.trktype) === 'string' ? data.trktype : '';
    kwkArgs.progid    = getType(data.progid) === 'number' ? data.progid : 0;
    kwkArgs.ptag      = kwkFirePtag;
    log('KWKGTM debug - full args list before trk =', kwkArgs);
    if (setInWindow('kwkgtm', kwkArgs, true)) {
      injectScript('https://img.metaffiliation.com/na/na/res/trk/kwkgtm.js');
      kwkFirePtag = false;
    }
    else {
      log('KWKGTM debug - unable to inject kwkgtm.js for firing conversion');
    }
  }
}

if (kwkFirePtag) {
  if (getType(callInWindow('KWKUniJS.ptag',{})) !== 'undefined') {
    log('KWKGTM debug - Ptag fired');
  }
  else {
    kwkArgs.progid = getType(data.progid) === 'number' ? data.progid : 0;
    kwkArgs.ptag   = kwkFirePtag;
    log('KWKGTM debug - full args list before ptag =', kwkArgs);
    if (setInWindow('kwkgtm', kwkArgs, true)) {
      injectScript('https://img.metaffiliation.com/na/na/res/trk/kwkgtm.js', data.gtmOnSuccess, data.gtmOnFailure, 'kwkgtm');
    }
    else {
      log('KWKGTM debug - unable to inject kwkgtm.js for firing Ptag');
    }
  }
}

data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "all"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "kwkgtm"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "KWKUniJS.conversion"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "KWKUniJS.dclick"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "KWKUniJS.clicktime"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "KWKUniJS.ptag"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_cookies",
        "versionId": "1"
      },
      "param": [
        {
          "key": "cookieAccess",
          "value": {
            "type": 1,
            "string": "specific"
          }
        },
        {
          "key": "cookieNames",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "kwku"
              },
              {
                "type": 1,
                "string": "kwkdebug"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://*.metaffiliation.com/"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 08/07/2020 à 11:10:26
